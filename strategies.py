...
_grid_smart_direction_cache = {}  # symbol -> {"direction": ..., "fetched_at": ...}


async def get_smart_direction(symbol):
    """Fuer 'Smart Mode': schaut sich die 24h-Preisaenderung an und schlaegt eine
    Richtung vor (Momentum-Annahme: laeuft der Preis, geht der Trend weiter).
    Wird 5 Minuten gecacht, um nicht bei jedem Preis-Tick neu abzufragen."""
    cached = _grid_smart_direction_cache.get(symbol)
    ...
    if pair:
        try:
            ...
            debug_log(f"⚠️ [{symbol}] Smart-Mode 24h-Abfrage fehlgeschlagen", {"error": str(e)})

    _grid_smart_direction_cache[symbol] = {"direction": direction, "fetched_at": time.time()}
    return direction
...
async def on_price_update(symbol, price):
    ...
    if st["position"] is None:
        ...
        direction_mode = cfg.get("grid_direction_mode", "neutral")
        ...
        if direction_mode == "long_only":
            ...
            if crossed_down or crossed_up:
                smart_dir = await get_smart_direction(symbol)
                if smart_dir == "long" and crossed_down:
                    await execute_entry(symbol, "long", price, is_add_on=False)
                elif smart_dir == "short" and crossed_up:
                    await execute_entry(symbol, "short", price, is_add_on=False)
                # falls die 24h-Richtung nicht zur gekreuzten Seite passt, wird gewartet
        else:  # "neutral" (Standard, bisheriges Verhalten)
            if crossed_down:
                await execute_entry(symbol, "long", price, is_add_on=False)
            elif crossed_up:
                await execute_entry(symbol, "short", price, is_add_on=False)
    ...
    base_grid_step_abs = compute_step_abs(st["last_entry_price"] or st["avg_entry_price"], cfg, "grid")
    # Deviation Multiplier: jeder weitere Nachkauf braucht einen groesseren Abstand
    # als der vorherige (1.0 = fix, wie bisher; >1.0 = wachsend, wie Dextrabot's DCA-Bot)
    deviation_mult = cfg.get("grid_deviation_multiplier", 1.0)
    grid_step_abs = base_grid_step_abs * (deviation_mult ** st["entry_count"])
    # Size Multiplier: jeder weitere Nachkauf ist groesser als der vorherige (1.0 = gleich gross)
    size_mult = cfg.get("grid_size_multiplier", 1.0) ** st["entry_count"]
    max_nachkauf = cfg["max_nachkauf"]

    # Optionaler Notausstieg: schliesst die GESAMTE Position, wenn der Verlust vom
    # Ø-Einstieg aus zu groß wird - unabhaengig von Nachkauf-Stufen (0 = deaktiviert)
    emergency_sl_pct = cfg.get("grid_emergency_sl_pct", 0)
    if emergency_sl_pct > 0:
        pnl_pct = (price - st["avg_entry_price"]) / st["avg_entry_price"] * 100 if st["position"] == "long" \
            else (st["avg_entry_price"] - price) / st["avg_entry_price"] * 100
        if pnl_pct <= -emergency_sl_pct:
            await execute_exit(symbol, price, "NOTAUSSTIEG")
            return
    ...
    if st["position"] == "long":
        if price >= st["avg_entry_price"] + tp_step_abs:
            ...
            await execute_entry(symbol, "long", price, is_add_on=True, size_multiplier=size_mult)
    elif st["position"] == "short":
        if price <= st["avg_entry_price"] - tp_step_abs:
            await execute_exit(symbol, price, "TP")
        elif bot_active and price >= st["last_entry_price"] + grid_step_abs and (max_nachkauf == 0 or st["entry_count"] < max_nachkauf):
            await execute_entry(symbol, "short", price, is_add_on=True, size_multiplier=size_mult)
